...
import blockBoxLogo from '../assets/images/boxblock.png'
...
const Header = () => {
  ...
  const welcomeUser = (userName = '', toastHandler = toast) => {
    toastHandler.success(
      `Bem vindo(a) ${userName !== 'Unnamed' ? ` ${userName}` : ''}!`,
      {
        style: {
          background: '#04111d',
          color: '#fff',
  ...
  return (
    <nav className="border-gray-200 px-2 sm:px-4 py-2.5 rounded dark:bg-gray-900 mt-4">
      <div className="container flex flex-wrap justify-between items-center mx-auto">
        <Link href="/">
          <Image src={blockBoxLogo} height={40} width={230} />
        </Link>
        ...
        <div className="hidden w-full md:block md:w-auto" id="navbar-default">
          <ul className="flex flex-col p-4 mt-4  rounded-lg border border-gray-100 md:flex-row md:space-x-8 md:mt-0 md:font-medium md:border-0 dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700 font-sora">
            <li>
              <Link href="/colecoes" className="block py-2 pr-4 pl-3 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white" aria-current="page">
                Quente
              </Link>
            </li>
            <li>
              <Link href="/colecoes" className="block py-2 pr-4 pl-3 text-[#7780A1] rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent" aria-current="page">
                Ajuda
              </Link>
            </li>
            <li>
              <Link href="/colecoes/criar" className="block py-2 pr-4 pl-3 text-[#7780A1] rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent" aria-current="page">
                Criar
              </Link>
            </li>
          </ul>
        ...
        {!address && 
          <button type="button" className="hidden md:inline-flex  text-white bg-[#2557D6] hover:bg-[#2557D6]/90 focus:ring-4 focus:ring-[#2557D6]/50 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#2557D6]/50 mr-2 mb-2" onClick={async () => { await connectToWallet() }}>
          ...
          <div className='flex flex-row'>
            <CgProfile />
            <MdExitToApp onClick={disconnectWallet} />
          </div>
        }
      </div>
    </nav>
